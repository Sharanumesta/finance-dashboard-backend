...
/**
 * Build common where clause based on user role
 */
...
/**
 * Returns income and expense totals grouped by calendar month.
 */
...
/**
 * Returns income and expense totals grouped by week.
 */
const getWeeklyTrends = async (user, weeks = 8) => {
  ...
  for (const t of transactions) {
    ...
    if (t.type === "INCOME") {
      weekMap[key].income += amount;
    } else {
      weekMap[key].expense += amount;
    }
  }
